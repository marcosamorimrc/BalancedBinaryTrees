...
@interface AVL () <UIScrollViewDelegate>{
...
-(void)drawNodeInSearch:(BinNode<TreeNode, compare_to<TreeNode>>)node :(CGRect)frame :(int)searchedValue{
    ...
    if(  node.GetElement() != NULL){
        ...
        if(  (node.GetLeft()->GetElement() == NULL && node.GetElement().getData() > searchedValue) ||
             (node.GetRight()->GetElement() == NULL &&  node.GetElement().getData() < searchedValue)){
